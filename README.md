...
Чтобы инициализировать сцену и привязать ее к canvas-элементу, используется команда:
    ...
    var width = 640,  // ширина
        height = 480; // высота
    scene.init(width, height [, userFunction]);

, где [, userFunction] - необязательный агрумент, являющийся пользовательской функцией, которая выполнится тогда, когда документ будет
готов на 100% и все слои будут добавлены к игровой сцене.
...
, где - io.readKey() - функция, необходимая для назначения обработчика ввода текста.
Основной и единственный аргумент - это функция, которая тоже принимает единственный аргумент - введенный символ.
