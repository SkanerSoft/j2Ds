...
После этого в объекте res у вас будет доступен один метод:
...
* files - список всех внешних файлов
...
Если вам требуется выполнить код, и вы знаете, что в нем могут быть ошибки, но при этом работу игрового состояния прерывать нельзя, вы можете воспользоваться методом run:

    err.run(function() {
     /*Этот код выполнится, но если возникнет ошибка, это не прервет выполнение игрового цикла*/
    });

Так же есть возможность выполнить строку кода:

    err.runString('var x = 15; console.log(x);');

В случае успешного выполнения в консоли выведется 15, если же код будет написан с ошибкой, движок об этом сообщит, а так же скажет, где именно
замечена ошибка.
...
При необходимости что-либо вывести в консоль предпочтительно использовать метод err.show:
    ...
    err.show(x, 'red'); // Выведет в консоль значение переменной x, цвет маркера - красный
