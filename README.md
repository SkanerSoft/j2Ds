...
> Примечание: если нужно скрыть курсор, можно создать полностью прозрачное изображение и применить его в качестве курсора.
...
* breforeInit - перед инициализацией
* afterInit - после инициализзации, но ДО формирования документа
* loadedDOM - сразу же после формирования документа
* beforeSceneStart - перед запуском игрового цикла
* afterSceneStart - сразу же после его запуска
 
Следующие два события срабатывают во время выполнения игры 
 
* [writeModeKeyPress](#inputMode) - при вводе любого символа с клавиатуры в write-mode режиме
* changedGameState - при смене игрового состояния командой setGameState
...
Для этого есть специальная возможность считывать вводимые с клавиатуры символы:
    ...
    j2ds.on('writeModeKeyPress', function (symbol) {
