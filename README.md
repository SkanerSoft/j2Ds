...
соответствующих классов или объектов. Получить к ним достум можно при помощи Менеджера математики:
...
В игровых состояниях описывается вся игровая логика, игровые уровни, меню и поведение сех основных объектов.
...
> Примечание: newGameState - имя игрового состояния, созданного чеерз менеджер игровых состояний.
...
## <a name="sound"></a> Музыка (Звуки)

В своей игре вы так же можете использовать звуковое сопровождение. Для этого в j2Ds предусмотрен соответствующий Менеджер:

    var snd = j2ds.getSoundManager();

Он позволяет вам загружать музыкальные файлы, и воспроизводить их при необходимости.

Для управления в не мпредусмотрены следующие методы:

    var id = 'soundStep', // звук шагов
        files = ['sound/step.mp3', 'sound/step.ogg']; // Массив с файлами
        volume = 0.5; // Начальная громкость
    snd.load(id, files);

Так как не все форматы поддерживают разные браузеры, то предпочтительно ипользовать два формата: mp3 и ogg. Подробнее о поддержках
...
Чтобы обратиться к элементу, его можно инициализировать как ссылку в начале, присвоив результат команды load переменной, либо использовать get:
    ...
    var soundStep = snd.load(id, files); // Так

    snd.get('soundStep').метод(); // либо так
    
### Воспроизведение

Для однократного воспроизведения звука теперь достаточно просто вызвать метод play():

    soundStep.play(unlock); // unlock нужен, если требуется принудительное воспроизведение, параметр необязательный
...
Чтобы приостановить воспроизведение:
    ...
    soundStep.pause(lock); // lock нужен, если требуется заблокировать файл для воспроизведение, параметр необязательный

Для полной остановки воспроизведения:

    soundStep.stop(lock); // lock нужен, если требуется заблокировать файл для воспроизведение, параметр необязательный
...
Вы в любой момент можете проверить текущее состояние воспроизведения:
    ...
    soundStep.getState() - play | pause | stop
...
Если необходимо, вы можете изменить громкость воспроизведения любого файла:
    ...
    var vol = 0.2; // Значение громкости может варьироваться от 0 (нет звкка) до 1 (максимум)

    soundStep.setVolume(vol)
...
Каждый метод (play, pause, stop) может принимать один необязательный параметр, принужнающий выполнить команду.
Например, если в игровом цикле вы вызовите метод loop(), то остановить файл командой stop() не получится.
Чтобы остановить зацикленное вопспроизведение нужно воспользоваться командой stop(true) с параметром true, 
...
Вы так же можете воспользовать глобальными командами воспроизведения, остановки и паузы (за исключением loop).
...
пальцев, участующих в событии, а так же координаты каждого нажатия:
